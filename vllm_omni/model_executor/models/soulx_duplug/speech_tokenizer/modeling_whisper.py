...
except Exception:  # pragma: no cover - encoder-only runtime does not need generation helpers
    class WhisperGenerationMixin:  # type: ignore[no-redef]
        pass
...
class WhisperVQEncoder(WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperVQConfig):
        ...
        if config.quantize_encoder_only:
            if (
                ...
                or config.encoder_causal
            ):
                print("Apply causal attention")
            else:
                print("Apply block causal attention")
                print(f"block size: {config.quantize_causal_block_size}")
    ...
    def get_block_causal_attention_mask(self, attention_mask, block_size=50):
        ...
        if self.config.encoder_causal:
            # print("apply causal mask")
            block_causal_mask = causal_mask & attention_mask[:, None, :]
    ...
    def _retrieve_init_tokens(
        ...
        if self.config.skip_language_detection:
            return torch.as_tensor(
                [[generation_config.decoder_start_token_id] for _ in range(batch_size)],
                dtype=torch.long,
                device=self.device,
            ).expand(batch_size, -1)
        else:
            return super()._retrieve_init_tokens(
                input_features,
                batch_size,
                generation_config,
                config,
